...
import ОurWork from "../our_works/our_works";
import ClientsLogos from "../clients_logos/ClientsLogos";

const Work = ({ state, actions, libraries }) => {
  // Get information about the current URL.
  console.log(libraries.source.handlers);
  const data = state.source.get(state.router.link);
  console.log(state.router.link);
  // Get the data of the post.
  const work = state.source[data.type][data.id];
  console.log(work);
  ...
  return data.isReady  ? (
    <ArticleContainer>
      <div className="post-title">
        <Title dangerouslySetInnerHTML={{ __html: work.title.rendered }} />
      </div>

      {/* Render the content using the Html2React component so the HTML is processed
       by the processors we included in the libraries.html2react.processors array. */}
      <div className="container">
        <Html2React html={work.content.rendered} />
        <ОurWork page="work"/>
        <ClientsLogos data={work.logos} />
      </div>
    </ArticleContainer>
  ) : null;
};
