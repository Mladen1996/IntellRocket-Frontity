...
import ServicesApproach from "../services_approach/ServicesApproach";
...
const Services = ({ state, actions, libraries }) => {
  ...
  const services = state.source[data.type][data.id];
  ...
  return data.isReady ? (
    <ArticleContainer>
      <div className="post-title">
        <Title dangerouslySetInnerHTML={{ __html: services.title.rendered }} />
      </div>

      {/* Render the content using the Html2React component so the HTML is processed
       by the processors we included in the libraries.html2react.processors array. */}
      <Content>
        <Html2React html={services.content.rendered} />
        <SharedSectionHero data={services.first_section} />
        <ServicesList data={services.crb_sections} />
        <ServicesApproach data={services.approach} />
        <SectionQuote />
      </Content>
    </ArticleContainer>
  ) : null;
