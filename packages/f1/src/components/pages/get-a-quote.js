...
const GetAQuote = ({ state, actions, libraries }) => {
  // Get information about the current URL.
  
  const data = state.source.get(state.router.link);
  console.log(state.router.link);
  // Get the data of the post.
  const getaquote = state.source[data.type][data.id];
   ...
   */
  useEffect(() => {
    actions.source.fetch("/get-a-quote/");
    List.preload();
  }, []);
  ...
  return data.isReady  ? (
    <ArticleContainer>
      <div className="post-title">
        <Title dangerouslySetInnerHTML={{ __html: getaquote.title.rendered }} />
      </div>
      ...
      <div className="container">
        ...
        <GetQuote data={getaquote.contact} />
        <Html2React html={getaquote.content.rendered} />
      </div>
    </ArticleContainer>
  ) : null;
};

export default connect(GetAQuote);
